...
void SceneRender::CreateDeviceDependentResources()
{
	...
	if (m_loadingComplete == false)
	{
		...
		m_model = Model::CreateFromSDKMESH(L"Assets\\tiny.sdkmesh");//, device);
		{
			...
			// C:\\Users\\hot3dx-home\\Source\\App1\\x64\\Debug\\App1\\AppX\\
...
void App1::SceneRender::SetStartCameraVars()
{
...
void App1::SceneRender::MouseCursorRender(float positionX, float positionY)
{
	if (m_tracking)
	{
...
void App1::SceneRender::DrawPointsOne(XMVECTOR intersect, float positiontX, float positionY)
{
	if (m_bLButtonDown == true)
	{
		if (m_fPointSpace > 0.0F) {
...
void App1::SceneRender::Initialize()
{
	// Not Yet Used!!
	//----------------------------------------------------------------------
...
void App1::SceneRender::OnLButtonDown(UINT nFlags, XMFLOAT2 point)
{
	
}

void App1::SceneRender::OnRightButtonDown(UINT nFlags, XMFLOAT2 point)
{
	
}

void App1::SceneRender::OnMouseMove(UINT nFlags, XMFLOAT2 point)
{
...
bool SceneRender::Render()
{
	...
	m_sprites->Draw(m_resourceDescriptors->GetGpuHandle(Descriptors::WindowsLogo), GetTextureSize(m_texture2.Get()),
		XMFLOAT2(1, 1));// 10, 75));
	m_font->DrawString(m_sprites.get(), L"Hot3dx App1 DirectXToolKitXaml12 App Sample Dev Res", XMFLOAT2(270, 30), Colors::Yellow);
