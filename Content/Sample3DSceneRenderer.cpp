...
void Sample3DSceneRenderer::CreateWindowSizeDependentResources()
{
	...
	if (!m_isRightHanded)
	{
		perspectiveMatrix = DirectX::XMMatrixPerspectiveFovLH(
			fovAngleY,
			aspectRatio,
			0.01f,
			100.0f
		...
		perspectiveMatrix = DirectX::XMMatrixPerspectiveFovRH(
			fovAngleY,
			aspectRatio,
			0.01f,
			100.0f
	...
	if (!m_isRightHanded)
	{
		eyes = { 0.0f, 0.7f, -10.0f, 0.0f }; // away 10.0f
	}
	else {

		eyes = { 0.0f, 0.7f, 10.0f, 0.0f }; // away 10.0f
	}
	static const XMVECTORF32 eye = { eyes.f[0], eyes.f[1], eyes.f[2], eyes.f[3] };
	static const XMVECTORF32 at = { 0.0f, -0.1f, 0.0f, 0.0f };
	static const XMVECTORF32 up = { 0.0f, 1.0f, 0.0f, 0.0f };

	if (!m_isRightHanded)
	{
		XMStoreFloat4x4(&m_constantBufferData.view, DirectX::XMMatrixTranspose(DirectX::XMMatrixLookAtLH(eye, at, up)));
	}
	else {
...
void Sample3DSceneRenderer::Rotate(float radians)
{
	...
	XMStoreFloat4x4(&m_constantBufferData.model, DirectX::XMMatrixTranspose(DirectX::XMMatrixRotationY(radians)));
}
...
void App1::Sample3DSceneRenderer::ReleaseDeviceDependentResources()
{
...
bool Sample3DSceneRenderer::Render()
{
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		m_commandList->ClearRenderTargetView(renderTargetView, DirectX::Colors::CornflowerBlue, 0, nullptr);// Black, 0, nullptr); //CornflowerBlue, 0, nullptr);// 
...
App1::VertexPositionColor* App1::Sample3DSceneRenderer::InitVB2(App1::VertexPositionColor* cubeVerts, int* numVerts)
{
...
// (from the default pool) to hold all our 3 custom vertices. We also
	...
	int size = sizeof(App1::VertexPositionColor);
	
	std::vector<App1::VertexPositionColor> cubeVertices(vCount[0]);
	//(App1::VertexPositionColor*)malloc(vCount[0] * sizeof(App1::VertexPositionColor));
